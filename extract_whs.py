...
if __name__=="__main__":
  ...
  parser.add_argument('-o', '--output_dir', type=str, default='patches3/data_whs', help='output directory')
  ...
  parser.add_argument("-s", "--patch_size", type=int, length=2, default=[224, 224], help="Patch size")
  ...
  if True:
    ...
    for modal in [ "ct", "mr"]:
      ...
      input_dir = os.path.join(args.input_dir, modal + '_train')
      ...
      for fold_idx, (train_files, test_files) in enumerate(folds):
        ...
        for mode in ("train", "test"):
          ...
          df = extract( input_dir, targeted_files, modal, mode, out_base_dir=args.output_dir + str(fold_idx+1) + out_base_dir_suffix, df_train = df_train, fold=fold_idx+1, sim_dir=sim_dir, debug = args.debug, csv_suffix = out_base_dir_suffix, overlap_dir=((args.simct if args.simct is not None else args.simmr) if args.overlaps else None), perform_save = True) 
          if mode=="train":
            df_train = df
